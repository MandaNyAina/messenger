...
const NewMessage = () => {
    ...
    const onSubmit = async(e) => {
        e.preventDefault()
        let infoPart = partner.split("-")
        const newMessage = {
            "content": sender,
            "userIDSend": read_cookie("user_app_react"),
            "userIDReceive": infoPart[0]
        }
        if (sender !== "" && infoPart[0] !== "") {
            const save = await fetch("/message/send",{
                method: "POST",
                headers: {"Content-type":"application/json"},
                body: JSON.stringify(newMessage)
            })
            window.location = "/chat/"+infoPart[0]+"/"+infoPart[1]
        }
    }
    const getList = async() => {
        ...
        setChat(listValue)
        if (user.length === 0){
            const getUserList = await fetch("/user")
            const userList = await getUserList.json()
            setUser(userList)
        }
    }
    ...
    return (
        <div>
            ...
            <div className="col-sm-9 p-2 sticky-top" style={{backgroundColor: "white", borderBottom:"1px solid #cfcfcf", marginLeft: "25%"}}>
                <select className="form-control form-control-sm" value={partner} onChange={e => setPartner(e.target.value)}>
                    <option value="" >Destinataire ici</option>
                    {user.map(item => {
                        if (item._id !== read_cookie("user_app_react")) {
                            return (
                                <option key={item._id} value={item._id+"-"+item.pseudo}>{item.pseudo}</option>
                            )
                        } else {
